...
class TodoRoomRepositoryTest {
    ...
    fun test_getUpcomingTodoCountSingleMatch() {
        ...
        db.todoDao().insert(Todo("1", "Todo 1", now + day, false, now))
    ...
    @Test
    fun test_getUpcomingTodoCountMultipleMatch() {
        val dao = spy(db.todoDao())
        db.todoDao().insert(Todo("6", "Todo 6", now - day, false, now))
        db.todoDao().insert(Todo("1", "Todo 1", now + day, false, now))
        db.todoDao().insert(Todo("7", "Todo 7", now + day * 2, false, now))
        db.todoDao().insert(Todo("4", "Todo 4", now + day, true, now))
    ...
    @Test
    fun test_getUpcomingTodoCountNoMatch() {
        val dao = spy(db.todoDao())
        db.todoDao().insert(Todo("6", "Todo 6", now - day, false, now))
        db.todoDao().insert(Todo("1", "Todo 1", now + day, true, now))
        db.todoDao().insert(Todo("4", "Todo 4", now + day, true, now))
        val repo = TodoRoomRepository(dao)
        val expected = 0
    ...
    fun test_getAllTodosMultiple() {
        val dao = spy(db.todoDao())
        val testTodo = Todo("6", "Todo 6", now - day, false, now)
